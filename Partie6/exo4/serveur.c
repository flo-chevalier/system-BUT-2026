...
typedef struct {
    int socket;
    struct sockaddr_in coordonnees;
    char pseudo[32];
} Client;
...
void* fonctionChatPourThread(void* arg) {
    Client* client = (Client*) arg;
    char message[MAX_BUFFER];
    char messageEnvoi[MAX_BUFFER];

    printf("Client connecté - %s:%d\n",
           inet_ntoa(client->coordonnees.sin_addr),
           ntohs(client->coordonnees.sin_port));

    // on attend le pseudo
    int nbRecu = recv(client->socket, message, MAX_BUFFER, 0);
    if (nbRecu > 0) {
        message[nbRecu] = 0;
        strcpy(client->pseudo, message);
    }
    ...
    while (1) {
        ...
        if (nbRecu > 0) {
            message[nbRecu] = 0;
            printf("[Serveur] Reçu de %s : %s\n", client->pseudo, message);
        ...
        send(client->socket, messageEnvoi, strlen(messageEnvoi), 0);
        printf("[Serveur] Renvoyé à %s : %s\n", client->pseudo, messageEnvoi);
    }

    printf("\n[Serveur] Fermeture de la connexion...\n");
    shutdown(client->socket, SHUT_RDWR); // On n'envoie plus
...
int main() {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        ...
        clients[i].socket = socketCommunication;
        clients[i].coordonnees = coordonneesClient;
        pthread_create(&threads[i], NULL, fonctionChatPourThread, &clients[i]);
